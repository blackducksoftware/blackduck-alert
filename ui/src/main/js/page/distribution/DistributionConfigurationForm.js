...
const DistributionConfigurationForm = ({
    ...
    const [channelModel, setChannelModel] = useState(FieldModelUtilities.createEmptyFieldModel([], CONTEXT_TYPE.DISTRIBUTION, AZURE_BOARD_INFO.key));
    const [specificChannelModel, setSpecificChannelModel] = useState(FieldModelUtilities.createEmptyFieldModel([], CONTEXT_TYPE.DISTRIBUTION, AZURE_BOARD_INFO.key));
    ...
    const [selectedChannel, setSelectedChannel] = useState(AZURE_BOARD_INFO.key);
    ...
    useEffect(() => {
        const channelFieldModel = (formData && formData.fieldModels)
            ...
            : FieldModelUtilities.createEmptyFieldModel([], CONTEXT_TYPE.DISTRIBUTION, AZURE_BOARD_INFO.key);
    ...
    if (!FieldModelUtilities.hasKey(channelModel, DISTRIBUTION_COMMON_FIELD_KEYS.channelName)) {
        onChannelSelectChange({ target: { name: DISTRIBUTION_COMMON_FIELD_KEYS.channelName, value: [AZURE_BOARD_INFO.key] } });
    }
