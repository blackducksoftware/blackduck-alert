export const JIRA_CLOUD_INFO = {
    key: 'channel_jira_cloud',
    url: 'jira',
    label: 'Jira Cloud'
};

export const JIRA_CLOUD_GLOBAL_FIELD_KEYS = {
    url: 'jira.cloud.url',
    emailAddress: 'jira.cloud.admin.email.address',
    accessToken: 'jira.cloud.admin.api.token',
    disablePluginCheck: 'jira.cloud.disable.plugin.check',
    configurePlugin: 'jira.cloud.configure.plugin'
};

export const JIRA_CLOUD_DISTRIBUTION_FIELD_KEYS = {
    comment: 'channel.jira.cloud.add.comments',
    fieldMapping: 'channel.jira.cloud.field.mapping',
    issueCreator: 'channel.jira.cloud.issue.creator',
    issueType: 'channel.jira.cloud.issue.type',
    project: 'channel.jira.cloud.project.name',
    resolveWorkflow: 'channel.jira.cloud.resolve.workflow',
    reopenWorkflow: 'channel.jira.cloud.reopen.workflow'
};
