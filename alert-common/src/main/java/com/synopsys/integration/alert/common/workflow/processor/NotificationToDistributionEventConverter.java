...
public class NotificationToDistributionEventConverter {
    ...
    private Map<String, ConfigurationFieldModel> getGlobalFields(DescriptorKey descriptorKey) {
        try {
            List<ConfigurationModel> globalConfiguration = configurationAccessor.getConfigurationByDescriptorNameAndContext(descriptorKey.getUniversalKey(), ConfigContextEnum.GLOBAL);
