...
public class CommonDistributionConfiguration extends Configuration {
    ...
    public CommonDistributionConfiguration(@NotNull final UUID jobUUID, @NotNull final Map<String, ConfigurationFieldModel> keyToFieldMap) {
        ...
        filterByProject = getFieldAccessor().getBoolean(KEY_FILTER_BY_PROJECT).orElse(null);
        projectNamePattern = getFieldAccessor().getString(KEY_PROJECT_NAME_PATTERN).orElse(null);
        configuredProjects = getFieldAccessor().getAllStrings(KEY_CONFIGURED_PROJECT).stream().collect(Collectors.toSet());
    }
