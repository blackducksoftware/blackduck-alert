...
public abstract class RestDistributionChannel<E extends ChannelEvent> extends DistributionChannel<E> {
    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final ChannelRestConnectionFactory channelRestConnectionFactory;

    public RestDistributionChannel(final Gson gson, final AlertProperties alertProperties, final BlackDuckProperties blackDuckProperties, final AuditUtility auditUtility,
        final Class eventClass, final ChannelRestConnectionFactory channelRestConnectionFactory) {
        super(gson, alertProperties, blackDuckProperties, auditUtility, eventClass);
    ...
    public void sendMessage(final E event) throws IntegrationException {
        try (final RestConnection restConnection = channelRestConnectionFactory.createUnauthenticatedRestConnection(getApiUrl())) {
    ...
    public abstract List<Request> createRequests(final E event) throws IntegrationException;
