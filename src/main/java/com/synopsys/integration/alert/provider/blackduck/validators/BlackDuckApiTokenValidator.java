...
public class BlackDuckApiTokenValidator {
    ...
    private boolean isNotificationApiAllowed(BlackDuckHttpClient httpClient) {
        boolean valid = true;
        try {
            //Verifies if the user has access to /api/notificatitons endpoint. Since only the the ability to access the endpoint is tested returning actual notifications is not needed.
