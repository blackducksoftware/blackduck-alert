...
public class AuditJobRelation extends DatabaseRelation {
    ...
    public static java.lang.Void removeMe(final java.sql.Connection connection) throws java.sql.SQLException {
        try (final java.sql.Statement getAuditEntryStatement = connection.createStatement()) {
            ...
            while (resultSet.next()) {
                ...
                try (final java.sql.Statement insertIntoRelationStatement = connection.createStatement()) {
                    insertIntoRelationStatement.executeUpdate("INSERT INTO ALERT.AUDIT_JOB_RELATION (AUDIT_ID, JOB_ID) VALUES (" + auditId + ", " + jobId + ");");
                }
            }
        }
        return null;
