    ...
    public void handle(JobProcessingEvent event) {
        ...
        try {
            ...
            if (jobModel.isPresent()) {
                ...
                logger.debug("No distribution job found with id: {} [correlationId: {}]. ", jobId, correlationId);
            }
        } finally {
            logger.debug("Cleaning up distribution job mapping. [jobId: {}, correlationId: {}]", jobId, correlationId);
