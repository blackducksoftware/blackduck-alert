...
public class BlackDuckAccumulatorReader implements ItemReader<NotificationDetailResults> {
    private static final String ENCODING = "UTF-8";

    private final static Logger logger = LoggerFactory.getLogger(BlackDuckAccumulatorReader.class);

    private final BlackDuckProperties blackDuckProperties;
    private final String lastRunPath;

    public BlackDuckAccumulatorReader(final BlackDuckProperties blackDuckProperties) {
    ...
    private String findLastRunFilePath() {
        String path = "";
        try {
            final String configLocation = blackDuckProperties.getEnvironmentVariable(AlertEnvironment.ALERT_CONFIG_HOME.getVariableName());
    ...
    public NotificationDetailResults read() throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException {
        ...
        if (optionalRestConnection.isPresent()) {
            try (final RestConnection restConnection = optionalRestConnection.get()) {
                ...
                final HubServicesFactory blackDuckServicesFactory = blackDuckProperties.createBlackDuckServicesFactory(restConnection);
                ...
                final NotificationService notificationService = blackDuckServicesFactory.createNotificationService(true);
                final NotificationDetailResults notificationResults = notificationService.getAllNotificationDetailResultsPopulated(blackDuckBucket, startDate, endDate);
            ...
            logger.warn("Skipping the Black Duck accumulation, could not create the connection.");
        }
        return null;
    }
