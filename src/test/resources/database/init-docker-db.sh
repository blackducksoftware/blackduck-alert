...
if [ -z $volume_path ]
  then
      ...
      docker run -d --name "$container_name" -e POSTGRES_PASSWORD=$postgres_password --mount type=volume,source=$container_name-volume,destination=/var/lib/postgresql/data -p $exposed_port:5432 postgres:16.15-alpine
  else 
    docker run -d --name "$container_name" -e POSTGRES_PASSWORD=$postgres_password --mount type=bind,source=$volume_path,destination=/var/lib/postgresql/data -p $exposed_port:5432 postgres:16.15-alpine
fi
