...
public abstract class ActionsTest<R extends CommonDistributionConfigRestModel, GR extends ConfigRestModel, E extends DatabaseEntity, GE extends DatabaseEntity, DCA extends DistributionConfigActions<E, R>> {
    ...
    public void testGetConfig() throws AlertException {
        ...
        Mockito.when(configActions.configuredProjectsRepository.findOne(1L)).thenReturn(projectMockUtils.getProjectOneEntity());
        Mockito.when(configActions.configuredProjectsRepository.findOne(2L)).thenReturn(projectMockUtils.getProjectTwoEntity());
        Mockito.when(configActions.configuredProjectsRepository.findOne(3L)).thenReturn(projectMockUtils.getProjectThreeEntity());
        Mockito.when(configActions.configuredProjectsRepository.findOne(4L)).thenReturn(projectMockUtils.getProjectFourEntity());
        Mockito.when(configActions.distributionProjectRepository.findByCommonDistributionConfigId(Mockito.anyLong())).thenReturn(projectMockUtils.getProjectRelations());
    ...
    public void testSaveConfig() throws Exception {
        ...
        Mockito.when(configActions.configuredProjectsRepository.findByProjectName(projectMockUtils.getProjectOne())).thenReturn(projectMockUtils.getProjectOneEntity());
        Mockito.when(configActions.configuredProjectsRepository.findByProjectName(projectMockUtils.getProjectTwo())).thenReturn(projectMockUtils.getProjectTwoEntity());
        Mockito.when(configActions.configuredProjectsRepository.findByProjectName(projectMockUtils.getProjectThree())).thenReturn(projectMockUtils.getProjectThreeEntity());
        Mockito.when(configActions.configuredProjectsRepository.findByProjectName(projectMockUtils.getProjectFour())).thenReturn(projectMockUtils.getProjectFourEntity());
