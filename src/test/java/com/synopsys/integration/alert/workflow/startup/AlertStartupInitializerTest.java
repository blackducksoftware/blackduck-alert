...
public class AlertStartupInitializerTest {
    ...
    public void testInitializeConfigs() throws Exception {
        ...
        Mockito.when(baseDescriptorAccessor.getFieldsForDescriptor(Mockito.anyString(), Mockito.any(ConfigContextEnum.class))).thenReturn(new ArrayList<>(channelDescriptor.getAllDefinedFields(ConfigContextEnum.GLOBAL)));
    ...
    public void testSetRestModelValueCreate() throws Exception {
        ...
        final List<DescriptorActionApi> restApis = List.of(descriptorConfig);
        final ChannelDescriptor channelDescriptor = new HipChatDescriptor(null, null, null, null, new HipChatGlobalUIConfig());
        Mockito.when(descriptorAccessor.getFieldsForDescriptor(Mockito.anyString(), Mockito.any(ConfigContextEnum.class))).thenReturn(new ArrayList<>(channelDescriptor.getAllDefinedFields(ConfigContextEnum.GLOBAL)));
        final List<ChannelDescriptor> channelDescriptors = List.of(channelDescriptor);
        final List<ProviderDescriptor> providerDescriptors = List.of();
        final List<ComponentDescriptor> componentDescriptors = List.of();
        final DescriptorMap descriptorMap = new DescriptorMap(channelDescriptors, providerDescriptors, componentDescriptors, restApis);
