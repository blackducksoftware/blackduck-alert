...
public class HelmInstalledCondition implements ExecutionCondition {
    private final Logger logger = LoggerFactory.getLogger(getClass());

    @Override
    public ConditionEvaluationResult evaluateExecutionCondition(final ExtensionContext context) {
        Path presentWorkingDirectory = Paths.get("").toAbsolutePath();
        Executable executable = Executable.create(presentWorkingDirectory.toFile(), List.of("helm", "version"));
